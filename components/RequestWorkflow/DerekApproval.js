...
function DerekApprovalsPage({ email }) {
    const [pendingDerekApprovalRequests, setPendingDerekApprovalRequests] = useState([]);
    const [completedRequests, setCompletedRequests] = useState([]);
    const [deniedRequests, setDeniedRequests] = useState([]);
    const [currentCategory, setCurrentCategory] = useState('pending');

    useEffect(() => {
        // Fetch 'Pending Derek Approval' requests when the component mounts
    ...
    const fetchData = async (category) => {
        try {
            ...
            if (category === 'pending') {
                setPendingDerekApprovalRequests(response.data);
    ...
    const getRequestById = (requestId) => {
        ...
        if (currentCategory === 'pending') {
            requestArray = pendingDerekApprovalRequests;
    ...
    // Derek Approves Request
    // const handleDerekApproval = async (requestId) => {
    //     try {
    ...
    //     }
    // };
    const handleDerekApproval = async (requestId, selectedFromProject) => {
        const projectPart = selectedFromProject.split('-')[0].trim(); // Split and take the first part before '-'
    
        console.log('selectedFromProject:', selectedFromProject);
        console.log('projectPart:', projectPart);
    
        try {
            // Determine the API endpoint and set the initial request data
            let apiEndpoint;
            let requestData = { Request_ID: requestId };
    
            if (projectPart === 'Project Risk and Scope Contingency' || projectPart === 'CEO Contingency') {
                apiEndpoint = `https://44.217.202.126/api/alternate-approve/${requestId}`;
                // requestData.selectedFromProject = projectPart;
            } else {
                apiEndpoint = `https://44.217.202.126/api/respond-to-request/${requestId}`;
            }
    
            // Make a PUT request to the determined API endpoint with request data
    ...
    const moveRequestToCompleted = (requestId) => {
        const updatedRequests = pendingDerekApprovalRequests.filter(
        ...
        setPendingDerekApprovalRequests(updatedRequests);
    };
    ...
    return (
        <div>
            <Navbar email={email}/>
            <h2>Requests Pending Derek Approval</h2>
            <ToggleButtonGroup
            ...
            <table>
                ...
                {currentCategory === 'pending' &&
                        pendingDerekApprovalRequests.map((request) => (
                            (request.Status !== 'Request Denied'&& request.Status !== 'Request Approve Completed' && request.Status !== 'Pending Simon Approval') && (
                            ...
                            <td>
                                <div className='request-button-container'>
                                    <button
                                        ...
                                        onClick={() => handleDerekApproval(request.Request_ID, request['From Category/Project'])}
                                    >
                                        Approve
                                    </button>
                                    <button
                                ...
                                <td>
                                    <div className='request-button-container'>
                                        <button
                                            disabled
                                            className='request-button'
                                            onClick={() => handleDerekApproval(request.Request_ID)}
                                        >
                                            Approve
                                        </button>
                                        <button
...
export default DerekApprovalsPage;
