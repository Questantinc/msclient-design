...
function NickApprovalsPage({ email }) {
    const [pendingNickApprovalRequests, setPendingNickApprovalRequests] = useState([]);
    const [completedRequests, setCompletedRequests] = useState([]);
    const [deniedRequests, setDeniedRequests] = useState([]);
    const [currentCategory, setCurrentCategory] = useState('pending');

    useEffect(() => {
        // Fetch 'Pending Nick Approval' requests when the component mounts
    ...
    const fetchData = async (category) => {
        try {
            ...
            if (category === 'pending') {
                setPendingNickApprovalRequests(response.data);
    ...
    const getRequestById = (requestId) => {
        ...
        if (currentCategory === 'pending') {
            requestArray = pendingNickApprovalRequests;
...
// Nick Approves Request

const handleNickApproval = async (requestId) => {
    try {
        // Make a PUT request to /api/approve/Neil to approve the request and change the status
        const response = await Axios.put(`https://44.217.202.126/api/respond-to-neil/${requestId}`, {
    ...
    const moveRequestToCompleted = (requestId) => {
        const updatedRequests = pendingNickApprovalRequests.filter(
        ...
        setPendingNickApprovalRequests(updatedRequests);
    };
    ...
    return (
        <div>
            <Navbar email={email}/>
            <h2>Requests Pending Nick Approval</h2>
            <ToggleButtonGroup
            ...
            <table>
                ...
                {currentCategory === 'pending' &&
                        pendingNickApprovalRequests.map((request) => (
                            ...
                            <td>
                                <div className='request-button-container'>
                                    <button
                                        ...
                                        onClick={() => handleNickApproval(request.Request_ID, request.Amount)}
                                    >
                                        Approve
                                    </button>
                                    <button
                                ...
                                <td>
                                    <div className='request-button-container'>
                                        <button
                                            disabled
                                            className='request-button'
                                            onClick={() => handleNickApproval(request.Request_ID)}
                                        >
                                            Approve
                                        </button>
                                        <button
...
export default NickApprovalsPage;
